...
public class CoreAPITestIT extends TestUtil {
    ...
    public void validate() throws InterruptedException {
        ...
        FutureData<Validation> csdl = datasift.validate(csdlValid);
        ...
        FutureData<Validation> invalidValidation = datasift.validate(csdlInvalid);
    ...
    public void validateSynchronously() throws InterruptedException {
        Validation csdl = datasift.validate(csdlValid).sync();
        successful(csdl);
        assertTrue(csdl.isValid());
        Validation invalidValidation = datasift.validate(csdlInvalid).sync();
        assertFalse(invalidValidation.isValid());
    }

    @Test(timeout = Settings.TIMEOUT)
    public void compile() throws InterruptedException {
        datasift.compile(csdlValid).onData(new FutureResponse<Stream>() {
    ...
    public void compileSync() throws InterruptedException {
        Stream data = datasift.compile(csdlValid).sync();
        successful(data);
        assertNotNull(data.hash());
        assertNotEquals(data.hash(), "", "Stream hash should never be empty");
    }
    ...
    public void compileInvalidCSDL() throws InterruptedException {
        datasift.compile(csdlInvalid).onData(new FutureResponse<Stream>() {
            public void apply(Stream data) {
    ...
    public void compileInvalidCSDLSync() throws InterruptedException {
        Stream data = datasift.compile(csdlInvalid).sync();
        assertNotEquals(data.getError(), "", "Error message shouldn't be empty");
    }

    @Test(timeout = Settings.TIMEOUT)
    public void dpus() throws InterruptedException {
        FutureData<Stream> data = datasift.compile(csdlValid);
        //dpus can be fetched from a stream future
        FutureData<Dpu> dpusFromFuture = datasift.dpu(data);
        //or from an already fetched stream
        Dpu dpus = datasift.dpu(data.sync()).sync();
        successful(dpus);
        //Minimum DPU is 1
    ...
    @Test(timeout = Settings.TIMEOUT)
    public void balance() {
        datasift.balance().onData(new FutureResponse<Balance>() {
...
//        Balance data = datasift.balance().sync();
//        successful(data);
//        assertNotNull("Balance data must be returned", data);
//    }
    ...
    @Test(timeout = Settings.TIMEOUT)
    public void usage() {
        datasift.usage().onData(new FutureResponse<Usage>() {
...
//        Usage data = datasift.usage().sync();
//        successful(data);
//        assertNotNull(data.getStreams());
//    }
}
